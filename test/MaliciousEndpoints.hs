...
tallyNoQuorum vCfg@VotingConfig {voteAsset} = do
  votesUtxo <- utxosAt (upgradeScriptAddress vCfg)
  ...
  let Just upgradeScriptHash = toValidatorHash $ upgradeScriptAddress vCfg
  ...
  let rebuildVote ((_, utxo), datum) = mustPayToOtherScript upgradeScriptHash datum $ txOutValue (toTxOut utxo)
      ...
      votesLookups = (typedValidatorLookups (upgradeScriptInstance vCfg)) <> (unspentOutputs votesUtxo)
...
returnOtherVote vCfg = do
  ...
  voteUtxos <- utxosAt (upgradeScriptAddress vCfg)
  ...
  let voterPkh = unPaymentPubKeyHash voter
      ...
      lookups = typedValidatorLookups (upgradeScriptInstance vCfg)
